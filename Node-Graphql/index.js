...
const resolvers = {
	...
	Query: {
		...
		allPersons: async (root, args) => {
			...
			const byPhone = (personsFromApi) =>
				args.phone === "YES" ? personsFromApi.phone : !personsFromApi.phone;
	...
	Mutation: {
		...
		editPerson: async (root, args) => {
			try {
				...
				await axios.put(
					`http://localhost:3000/persons/${personsFromApi[personIndex].id}`,
					updatedPerson,
				);
